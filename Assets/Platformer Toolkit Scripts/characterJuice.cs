...
public class characterJuice : MonoBehaviour {
    public InspectorInfo info = new InspectorInfo( "Juice Component is NOT Complete", "If you want to use this, you'll need to either add Animation and ParticleSystem components or avoid the null reference exceptions when this tries to call them." );
    ...
    private void tiltCharacter()
    {
        ...
        Vector3 targetRotVector = new Vector3(0, 0, Mathf.Lerp(-gmtkSettingsSO.maxTilt, gmtkSettingsSO.maxTilt, Mathf.InverseLerp(-1, 1, directionToTilt)));

        //And then rotate the character in that direction
        if (myAnimator != null) myAnimator.transform.rotation = Quaternion.RotateTowards(myAnimator.transform.rotation, Quaternion.Euler(-targetRotVector), gmtkSettingsSO.tiltSpeed * Time.deltaTime);
    }
    ...
    private void checkForLanding()
    {
        if (!playerGrounded && jumpScript.onGround)
        {
            ...
            if (!landSqueezing && gmtkSettingsSO.landSqueezeMultiplier > 1)
            {
                StartCoroutine(JumpSqueeze( gmtkSettingsSO.landSquashSettings.x * gmtkSettingsSO.landSqueezeMultiplier, gmtkSettingsSO.landSquashSettings.y / gmtkSettingsSO.landSqueezeMultiplier, gmtkSettingsSO.landSquashSettings.z, gmtkSettingsSO.landDrop, false));
            }
    ...
    public void jumpEffects()
    {
        ...
        if (!jumpSqueezing && gmtkSettingsSO.jumpSqueezeMultiplier > 1)
        {
            StartCoroutine(JumpSqueeze( gmtkSettingsSO.jumpSquashSettings.x / gmtkSettingsSO.jumpSqueezeMultiplier, gmtkSettingsSO.jumpSquashSettings.y * gmtkSettingsSO.jumpSqueezeMultiplier, gmtkSettingsSO.jumpSquashSettings.z, 0, true));
